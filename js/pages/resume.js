...
export class ResumePage {
    ...
    render() {
        ...
        page.innerHTML = `
            <style>
                ...
                @media print {
                    @page {
                        margin: 0; /* Remove default browser margins for full-page A4 */
                    }
                    body, html {
                    ...
                    .resume-builder-page, .editor-container, .canvas-area {
                        height: auto !important;
                        overflow: visible !important;
                        display: block !important; /* override flex to prevent flexbox clipping */
                        background: white !important;
                    }
                    ...
                    .resume-canvas {
                        ...
                        transform: none !important; /* Ignore user zoom during print */
                        margin: 0 !important;
                        padding-top: 20mm !important; /* Add some top margin to print */
                    }
                }
            ...
            <div class="ai-modal-overlay" id="aiModal" style="display: none;">
                <div class="ai-modal">
                    ...
                    <div class="modal-actions">
                        <button class="btn-secondary" id="closeAiModal">Cancel</button>
                        <button class="btn-primary" id="generateAiBtn">
    ...
    async handleAIGeneration() {
        ...
        try {
            ...
            // Apply suggested template and layout if provided
            if (data.suggestedTemplate) {
                this.selectedTemplate = data.suggestedTemplate;
                localStorage.setItem('selectedTemplate', data.suggestedTemplate);
            }
            ...
            // Close modal
            this.element.querySelector('#aiModal').style.display = 'none';
    ...
    getTemplateStyles(layoutType) {
        ...
        const layouts = {
            ...
            'ats-standard': `
                ${base}
                .resume-template { padding: 40px; font-family: 'Arial', sans-serif; }
                .header-section { text-align: center; margin-bottom: 30px; border-bottom: none; }
                .resume-name { font-size: 28px; font-weight: 800; text-transform: uppercase; margin-bottom: 8px; }
                .contact-bar { justify-content: center; font-size: 14px; margin-bottom: 0; display: block; }
                .contact-line { margin-bottom: 4px; }
                .section-title { font-size: 18px; font-weight: 800; text-transform: uppercase; border-bottom: none; border-left: none; padding-left: 0; margin-top: 25px; margin-bottom: 12px; text-decoration: none; }
                .item-header { display: flex; justify-content: flex-start; gap: 10px; font-weight: 700; margin-bottom: 2px; }
                .item-title { font-size: 15px; }
                .item-subtitle { color: #333; font-size: 14px; font-weight: 600; }
                .item-description { font-size: 14px; line-height: 1.4; margin-top: 5px; }
                .declaration-section { margin-top: 40px; font-style: normal; border-top: 1px solid #eee; padding-top: 20px; }
                .ongoing-list { list-style: disc; margin-left: 20px; }
            `
        };
            ...
            <div class="content-grid">
                ${data.summary ? `
                    <div class="resume-section">
                        ...
                        <div class="item-description" style="text-align: center;" contenteditable="true" data-field="summary">${data.summary}</div>
                    </div>` : ''}
    ...
    renderATSStandard(data) {
        return `
            <div class="header-section">
                ...
                <div class="contact-bar">
                    <div class="contact-line">
                        <span contenteditable="true" data-field="personalInfo.phone">Phone: ${data.personalInfo.phone || ''}</span> | 
                        <span contenteditable="true" data-field="personalInfo.email">Email: ${data.personalInfo.email || ''}</span>
                    </div>
                    <div class="contact-line">
            ...
            ${this.renderStandardSection(data, 'education', 'EDUCATION')}
            ${this.renderStandardSection(data, 'experience', 'INTERNSHIP EXPERIENCE')}
            ${this.renderStandardSection(data, 'projects', 'PROJECTS')}
            ${this.renderStandardSection(data, 'certifications', 'CERTIFICATIONS AND ACHIEVEMENTS')}
    ...
    formatDescription(text) {
        ...
        // Handle array if AI sends it
        const content = Array.isArray(text) ? text.join('\n') : String(text);

        // If it looks like it has multiple points (new lines), render as list
        const points = content.split('\n').filter(p => p.trim());
        if (points.length > 1) {
            return `<ul class="description-list">${points.map(p => `<li>${p.replace(/^[•\-\*]\s*/, '')}</li>`).join('')}</ul>`;
        }
        return content;
    }
                ...
                ${items.map(item => `
                    <div class="section-item">
                        <div class="item-header">
                            ...
                            <span class="item-subtitle" contenteditable="${!this.isViewMode}" data-field="${sectionKey}.${item.id}.date">
                                ${(item.startDate || '')} ${item.endDate ? '- ' + item.endDate : ''}
                            </span>
                        </div>
                        <div class="item-subtitle" contenteditable="${!this.isViewMode}" data-field="${sectionKey}.${item.id}.${sectionKey === 'education' ? 'school' : 'company'}">
                            ${sectionKey === 'education' ? (item.school || '') : (item.company || '')} ${item.location ? '| ' + item.location : ''}
                        </div>
    ...
    renderCustomSections(data) {
        ...
        return data.customSections.map(section => `
            <div class="resume-section">
                <div class="section-title" contenteditable="${!this.isViewMode}" data-field="customSections.${data.customSections.indexOf(section)}.title">${section.title}</div>
                ${section.items.map((item, i) => `
                    <div class="section-item">
                        <div class="item-header">
                            <span class="item-title" contenteditable="${!this.isViewMode}" data-field="customSections.${data.customSections.indexOf(section)}.items.${i}.title">${item.title}</span>
                            <span class="item-subtitle" contenteditable="${!this.isViewMode}" data-field="customSections.${data.customSections.indexOf(section)}.items.${i}.subtitle">${item.subtitle}</span>
                        </div>
                        <div class="item-description" contenteditable="${!this.isViewMode}" data-field="customSections.${data.customSections.indexOf(section)}.items.${i}.description">${this.formatDescription(item.description)}</div>
                    </div>
                `).join('')}
                ${!this.isViewMode ? `
                <div class="add-item-btn-container">
